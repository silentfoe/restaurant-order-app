...
document.getElementById('display-order-total').addEventListener('click', (event) => {
    
    if(event.target.id){
        event.target.id === 'order-btn' ? payForFoodOrder() : removeFoodFromOrder(+event.target.id)
    } 
})

// submit button for the payment and calling the function to show the order is on the way
document.getElementById('payment-modal').addEventListener('submit', orderOnTheWay)
...
function orderOnTheWay(event) {
    ...
    document.getElementById('display-order-total').innerHTML = `Thanks, ${name}! Your order is on its way!`
    resetOrderProcess()
